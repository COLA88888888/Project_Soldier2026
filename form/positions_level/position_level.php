...
if ($o_id > 0) { 
    $where_extra .= " AND o_id = $o_id"; 
} elseif (!empty($o_ids_arr)) {
    $clean_ids = implode(',', $o_ids_arr);
    $where_extra .= " AND o_id IN ($clean_ids)";
}
...
<a href="../../form/officers/show_position_level_id.php?l_id=<?= $rowbox['l_id'] ?>&pk_id=<?= $pk_id ?>&pk_ids=<?= urlencode($pk_ids_str) ?>&o_id=<?= $o_id ?>&o_ids=<?= urlencode($o_ids_str) ?>&u_id=<?= $u_id ?>&d_id=<?= $d_id ?>" class="text-decoration-none">
