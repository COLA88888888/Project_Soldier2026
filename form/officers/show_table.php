...
if (!empty($title)) {
    ...
    $names = [];
    $ids_imploded = implode(',', $pk_ids_arr);
    $r_filter = mysqli_query($conn, "SELECT pk_name FROM panak WHERE pk_id IN ($ids_imploded)");
    while ($row_f = mysqli_fetch_assoc($r_filter)) {
        $names[] = $row_f['pk_name'];
    }
    $filter_title = " (ພະແນກ: " . htmlspecialchars(implode(', ', $names)) . ")";
} elseif ($u_id > 0) {
