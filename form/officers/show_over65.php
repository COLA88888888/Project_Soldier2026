...
echo "<button class='btn btn-success btn-sm '>{$interval->y} ປີ {$interval->m} ເດືອນ {$interval->d} ວັນ</button>";
?>
</td>
<td><?= htmlspecialchars($row['date_join_police']) ?></td>
<td>
    <?php
$birth_date = $row['date_join_police']; 
$today = new DateTime(); 
$birth = new DateTime($birth_date);

$interval = $birth->diff($today);

echo "<button class='btn btn-danger btn-sm '>{$interval->y} ປີ {$interval->m} ເດືອນ {$interval->d} ວັນ</button>";
?>
</td>
