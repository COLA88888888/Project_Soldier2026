...
$i = 1;
include('../../condb.php');
$stmt = $conn->prepare("SELECT a.*,b.* FROM positions_level as a, rank_position as b where a.l_id=b.l_id  ORDER BY b.r_id   DESC");
$stmt->execute();
...
<tr>
<td><?= $i++ ?></td>
<td><img src="../../form/positions_level/uploads/<?= htmlspecialchars($row['l_img']) ?>" alt="" style="width:250px; height:100px;"></td>
<td><?= htmlspecialchars($row['l_name']) ?></td>
<td><?= htmlspecialchars($row['r_years']) ?></td>
