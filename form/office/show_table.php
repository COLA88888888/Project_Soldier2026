...
<h3 class="card-title">ລາຍງານຂໍ້ມູນ ຫ້ອງການ</h3>
...
$i = 1;
include('../../condb.php');
$stmt = $conn->prepare("SELECT a.*,b.* FROM department as a, office as b where a.d_id=b.d_id  ORDER BY b.d_id  DESC");
$stmt->execute();
...
<td>
<a href="../officers/show_table.php?o_id=<?= $row['o_id'] ?>" class="btn btn-info btn-sm">
<i class="fas fa-users"></i> ເບິ່ງພະນັກງານ
</a>
</td>
<?php if ($_SESSION['role'] == "admin") { ?>
<td>
<a href="show_table.php?o_id=<?= $row['o_id'] ?>" class="btn btn-danger btn-sm delete">
<i class="fas fa-trash"></i> ລົບ
</a>
<a href="edit.php?o_id=<?= $row['o_id'] ?>" class="btn btn-primary btn-sm edit">
