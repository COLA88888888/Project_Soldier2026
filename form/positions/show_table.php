...
<h3 class="card-title">ລາຍງານຂໍ້ມູນ ໜ້າທີ່ຕຳແໜ່ງ</h3>
...
$i = 1;
include('../../condb.php');
$stmt = $conn->prepare("SELECT *FROM positions ORDER BY pt_id  DESC");
$stmt->execute();
...
<?php if ($_SESSION['role'] == "admin") { ?>
<td>
<a href="show_table.php?pt_id=<?= $row['pt_id'] ?>" class="btn btn-danger btn-sm delete">
<i class="fas fa-trash"></i> ລົບ
</a>
<a href="edit.php?pt_id=<?= $row['pt_id'] ?>" class="btn btn-primary btn-sm edit">
